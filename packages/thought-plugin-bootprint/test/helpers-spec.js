...
describe('the handlebars-helpers:', function() {
  describe('the codeblock-helper', function() {
    it('should wrap the block with at least three backticks', function() {
      return run('{{#codeBlock}}abc\n{{/codeBlock}}').then(result => expect(result).to.equal('```\nabc\n```'))
    })
  ...
  describe('the removeStart-helper', function() {
    it('should remove the bootprint-prefix', function() {
      return run(`{{shortModuleName name}}`, { name: 'bootprint-json-schema' }).then(result =>
        expect(result).to.equals('json-schema')
      )
    })

    it('should leave the name as is, if it does not start with bootprint', function() {
      return run(`{{shortModuleName name}}`, { name: 'howdy-good-fella' }).then(result =>
        expect(result).to.equals('howdy-good-fella')
      )
    })
  })
})
